...
class PageSheetExample: UIViewController {
    let button = UIButton()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        button.setTitle("Page Sheet", for: .normal)
    ...
    @objc func onLoadFlow() {
        ...
        FrigadeProvider.load(flowId: "flow_NP2Petdcsjxq613V") { result in
            ...
            case .success(let flow):
                flow.delegate = self
                flow.present(overViewController: self, presentationStyle: .pageSheet, animated: true)
            case .failure(let error):
                NSLog("Error loading flow. Reason: \(error.localizedDescription)")
            }
            
            self.button.setTitle("Page Sheet", for: .normal)
            self.button.isUserInteractionEnabled = true
        }
    }
}

extension PageSheetExample: FrigadeFlowDelegate {
